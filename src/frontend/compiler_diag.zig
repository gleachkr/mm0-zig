...
fn compilerErrorSummary(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.InvalidDummyAnnotation => "@dummy expects exactly one real rule binder name",
        error.UnknownDummyBinder => "@dummy target must be a real rule binder",
        error.DuplicateDummyBinder => "multiple @dummy annotations were attached to one rule binder",
